...
class Tank_auth
{
	 ...
	 * @return	bool
	 */
	function login($login, $password, $remember, $login_by_username, $login_by_email,$admin="",$userID="")
	{
		if ((strlen($login) > 0) AND (strlen($password) > 0)) {
			...
			if ($login_by_username AND $login_by_email) {
				...
				$get_user_func = 'get_user_by_email';
                          ...
                          if($admin!=""){$get_user_func='get_user_by_email_admin';}
                        if($userID!="" and $admin!=""){$get_user_func='get_user_by_email_userid_admin';}
                        

			if (!is_null($users = $this->ci->users->$get_user_func($login,$userID))) {	// login ok
				...
				if ($hasher->CheckPassword($password, $user->password)) {		// password ok
                    $users_profile = $this->ci->users->get_user_by_userid($user->id);
...
//                }
	...
	public static function comparex($data){
		return 'hello';
	}
}
