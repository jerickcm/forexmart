    ...
    <?= $this->load->view('verify/AV_tabs', NULL, TRUE);?>
        <br><br><br>
         <div class="table-responsive col-md-12" id="searchaccounts">
                    <input aria-controls="AccountsTable" class="form-control input-sm" placeholder="Account number" id="s_accountnumber" type="search" style="width: 200px; display: inline-block; height: 36px;">
...
<div class="modal fade" id="verifiedStat" tabindex="-1" role="dialog" aria-labelledby="" style="width: 100%">
    <div class="modal-dialog modal-lg round-0">
        <div class="modal-content round-0">
            <div class="modal-header round-0">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
...
$(document).on("click", ".declinedsingle", function () {
    ...
    $(document).on("click", "#search", function () {
        $('#loader-holder').show();
        var s_accountnumber = $('#s_accountnumber').val();
        var prvt = [];
        prvt["data"] = {s_accountnumber: s_accountnumber};
        pblc['request'] = $.ajax({
            dataType: 'json',
            url: site_url+"account-verification/get_verifiedtab_search",
            method: 'POST',
            data: prvt["data"]
        });
        pblc['request'].done(function( data ) {
