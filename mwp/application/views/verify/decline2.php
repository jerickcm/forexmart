...
<div class="tab-pane active" id="check-phone-password">
    ...
    <?= $this->load->view('verify/AV_tabs', NULL, TRUE);?>
        <br><br><br>
         <div class="table-responsive col-md-12" id="searchaccounts">
                    <input aria-controls="AccountsTable" class="form-control input-sm" placeholder="Account number" id="s_accountnumber1" type="search" style="width: 200px; display: inline-block; height: 36px;">
                    <button type="button" id="search" name="Search" class="tab-input-button green-input-button">Search</button>
         </div>
    <div  class="table-responsive col-md-12 table-container-holder table-container-border table-container-margin data-center-container" style="margin-top: 65px;">
...
<div class="modal fade" id="declineSingle" tabindex="-1" role="dialog" aria-labelledby="" style="width: 100%;">
...
<div class="modal fade" id="declineNotes" tabindex="-1" role="dialog" aria-labelledby="" style="width: 100%;">
    <div class="modal-dialog modal-lg round-0" id="declineNotesModal">
        <div class="modal-content round-0">
            <div class="modal-header round-0">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                <h4 class="modal-title">
...
<div class="modal fade" id="declineStat" tabindex="-1" role="dialog" aria-labelledby="" style="width: 100%;">
    <div class="modal-dialog modal-lg round-0">
        <div class="modal-content round-0">
            <div class="modal-header round-0">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
...
//        "ajax": {
//            "url": site_url+"account-verification/get-declined",
//            "type": "POST",
...
function UpdateRequest(){
    ...
    pblc['request'] = $.ajax({
        dataType: 'json',
        url: site_url+"account-verification/AV_Unverified_updaterequest",
        method: 'POST',
        data: prvt["data"]
    });
...
$(document).on("click", ".declinedsingle", function () {
    ...
    $(document).on("click", "#search", function () {
        $('#loader-holder').show();
        var s_accountnumber = $('#s_accountnumber1').val();
        var prvt = [];
        prvt["data"] = {s_accountnumber: s_accountnumber};
        pblc['request'] = $.ajax({
            dataType: 'json',
            url: site_url+"account-verification/get_declinedtab_search",
            method: 'POST',
            data: prvt["data"]
        });
        pblc['request'].done(function( data ) {
            $('#request').html(data.row);
            $('#loader-holder').hide();
        });
        pblc['request'].fail(function( jqXHR, textStatus ) {
            $('#loader-holder').hide();
...
$(document).on("click", ".declinednotes", function () {
    ...
    pblc['request'] = $.ajax({
        dataType: 'json',
        url: site_url+"account-verification/avdeclinenote",
        method: 'POST',
        data: prvt["data"]
    });
    pblc['request'].done(function( data ) {
