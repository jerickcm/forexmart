...
//        TimeFrame: 15,
    ...
    var TradesViewModel = {
        stopLimitOrders: ValidateObservableProperty(params.Data.StopLimitOrders, "StopLimitOrders", true),
        openTrades: ValidateObservableProperty(params.Data.OpenTrades, "OpenTrades", true)
    }
    ...
    self.SetChartObect = function (isShowHide) {
        (!isShowHide) ? (self.IsChartObjectsVisible === false) ? self.IsChartObjectsVisible = true : self.IsChartObjectsVisible = false : self.IsChartObjectsVisible = isShowHide;
        self.IsExplicitlySetChartObjects = true;
        self.TempIsChartObjectsVisible = self.IsChartObjectsVisible;
        SetToolBarGroupVisibility();
        ChartInstance.Resizemethod();
    ...
    self.SetChartDisplay = function (isShowHide) {
        (!isShowHide) ? (self.IsChartDisplayVisible === false) ? self.IsChartDisplayVisible = true : self.IsChartDisplayVisible = false : self.IsChartDisplayVisible = isShowHide;
        self.IsExplicitlySetChartDisplay = true;
        self.TempIsChartDisplayVisible = self.IsChartDisplayVisible;
        SetToolBarGroupVisibility();
        ChartInstance.Resizemethod();
    ...
    self.SetVisualTools = function (isShowHide) {
        (!isShowHide) ? (self.IsVisualToolsVisible === false) ? self.IsVisualToolsVisible = true : self.IsVisualToolsVisible = false : self.IsVisualToolsVisible = isShowHide;
        self.IsExplicitlySetVisualTools = true;
        self.TempIsVisualToolsVisible = self.IsVisualToolsVisible;
        SetToolBarGroupVisibility();
        ChartInstance.Resizemethod();
    ...
    function IndicatorCtxMenuCallBack(key, SettingsValues) {
        if (!key) return;
        IndicatorInstance.AllowAdd = true;
        IndicatorInstance.LoadOnBarsIndicator(key, ChartInstance.ChartOtherProperties.ChartData, Options, SettingsValues);
    ...
    function SetAllElementsNeededAndSetVariablesNeeded() {
        ...
        ChartInstance = new WC.CM.OHLCChart(ChartHolder, self.ResizeFunction, BarsViewModel, Options, self, params.CallBacks, QuotesViewModel, TradesViewModel);
    ...
    self.AddVisualTools = function (VTName, PropertySettings) {
        if (["TrendLine", "Pitchfork", "Ellipse", "Rectangle", "Text", "GannFan", "GannLine", "FibonacciArc", "FibonacciFan", "FibonacciTimezone", "ForexChannel"].indexOf(VTName) < 0) return;
        VTInstance.initDrawVT(VTName);
        var quarterWidth = ChartInstance.VisualToolsCanvas.clientWidth / 4;
        var quarterHeight = ChartInstance.VisualToolsCanvas.clientHeight / 4;
        var X1 = quarterWidth;
        var Y1 = quarterHeight;
        var X2 = ChartInstance.VisualToolsCanvas.clientWidth - quarterWidth;
        var Y2 = ChartInstance.VisualToolsCanvas.clientHeight - quarterHeight;

        VTInstance.FireMouseDown(X1, Y1);
        VTInstance.FireMouseUp();
        //VTInstance.tempVT.SettingsValues = $.extend(VTInstance.tempVT.SettingsValues, PropertySettings);
        Object.keys(VTInstance.tempVT.SettingsValues).map(function (prop) {
            if (PropertySettings[prop]) VTInstance.tempVT.SettingsValues[prop] = PropertySettings[prop];
        });
        if (VTName == "Text") ChartInstance.VisualToolsInput.value = PropertySettings.Text ? PropertySettings.Text : "VisualToolText";
        VTInstance.FireMouseDown(X2, Y2);
        VTInstance.FireMouseUp();

    }

    self.HideMenus = function () {
        self.ParentElement.find(".ChartMenu").hide();
        self.SetChartObect(false);
        self.SetChartDisplay(false);
        self.SetVisualTools(false);


    }
    self.ShowMenus = function () {
        self.ParentElement.find(".ChartMenu").show();
        self.SetChartObect(true);
        self.SetChartDisplay(true);
        self.SetVisualTools(true);
    }
};
