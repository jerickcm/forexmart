...
WC.CM.OHLCChart = function (Container, ChartFormResizingMethod, BarsViewModel, options, ChartFormInstance, Callbacks, QuotesViewModel) {
    ...
    function CheckOrdersData() {
        ...
        if (OHLCChart.ChartSettings.IsOrdersVisible) {
            if (WC.TM.TradesModuleVM.stopLimitOrders().length !== 0) {
                SubscribeToOrdersAndAddDataToLocalVisibleTradesOnChart();
            }
            if (OrdersSub !== null) {
                OrdersSub.dispose();
            }
            OrdersSub = WC.TM.TradesModuleVM.stopLimitOrders.subscribe(SubscribeToOrdersAndAddDataToLocalVisibleTradesOnChart);
        }
        else {
            if (OrdersSub !== null) {
                OrdersSub.dispose();
    ...
    function ArrowStyleTrades(X, Y, TradeType) {
        ...
        if (TradeType === "Buy") {
            OHLCChart.ctxTradesCanvas.strokeStyle = OHLCChart.ChartSettings.BuysColor;
            OHLCChart.ctxTradesCanvas.moveTo(X - 7, Y + 8);
            OHLCChart.ctxTradesCanvas.lineTo(X, Y);
            OHLCChart.ctxTradesCanvas.lineTo(X + 7, Y + 8);
        }
        else {
            OHLCChart.ctxTradesCanvas.strokeStyle = OHLCChart.ChartSettings.Sells;
        ...
        for (var i = 0; i < OpenTradesLength; i++) {
            ...
            if (StampPixelX >= X - 7 && StampPixelX <= X + 7) {
                ...
                if (TradeTypeToWord.includes('Buy')) {
                    Color = OHLCChart.ChartSettings.BuysColor;
                }
                else {
                    Color = OHLCChart.ChartSettings.Sells;
                }
    ...
    function SubscribeToOrdersAndAddDataToLocalVisibleTradesOnChart(NewValue) {
        var stopLimitOrdersLength = WC.TM.TradesModuleVM.stopLimitOrders().length;
        ...
        if (isReCreateDataStopLimitOrders) {
            ...
            for (var i = 0; i < stopLimitOrdersLength; i++) {
                ...
                if (WC.TM.TradesModuleVM.stopLimitOrders()[i].Symbol() === OHLCChart.ChartSettings.Symbol) {
                    if (parseFloat(WC.TM.TradesModuleVM.stopLimitOrders()[i].Rate()) >= OHLCChart.ComputationProperties.yMin && parseFloat(WC.TM.TradesModuleVM.stopLimitOrders()[i].Rate()) <= OHLCChart.ComputationProperties.yMax) {
                        var OrdersDataLength = OrdersData.length;
                        if (OrdersDataLength === 0) {
                            OrdersData.push(WC.TM.TradesModuleVM.stopLimitOrders()[i]);
                            SetElementsForOrders(WC.TM.TradesModuleVM.stopLimitOrders()[i].OrderId());
                            continue;
                        }

                        //Check for the Same OpenQuote Of the Data
                        for (var ii = 0; ii < OrdersDataLength; ii++) {
                            if (parseFloat(OrdersData[ii].Rate()) === parseFloat(WC.TM.TradesModuleVM.stopLimitOrders()[i].Rate())) {
                                IsDuplicate = true;
                                break;
                            }
                        }
    ...
    function DrawDashedLineStopLimitOrders() {
        ...
        for (var i = 0; i < OrdersDataLength; i++) {
            ...
            if (TradeTypeWord.includes('Sell')) {
                LineColor = OHLCChart.ChartSettings.Sells;
            }
            else {
                LineColor = OHLCChart.ChartSettings.BuysColor;

            }
            var YPositionLine = Math.round(OHLCChart.GetYaxis(parseFloat(OrdersData[i].Rate())));
            OHLCChart.drawDashedLine("Orders", 0, YPositionLine, OHLCChart.ComputationProperties.width, YPositionLine, [6, 7, 0], LineColor, 1.5);
    ...
    function SetPositionStopLimitOrders(ID, YPosition, Color, TradeTypeWord, Vol) {
        ...
        //Set The label Text
        ElementToSet.find('label.StopLimitOrdersDetails').text(TradeTypeWord + ' ' + Vol + ' Lot');
        ElementToSet = null;
    }
    ...
    function CheckHoveredTrades(Y) {
        ...
        for (var i = 0; i < OpenTradesDataLength; i++) {
            ...
            if (Y <= ComputedY + 7 && Y >= ComputedY - 7) {
                ...
                OHLCChart.BaseChartElement.find('[TradeID="' + TradeID + '"]').find('.TradesCloseButton').css({ display: "block" });
    ...
    function SetPositionTradesAndOrders(ID, YPosition, Color, TradeTypeWord, Vol) {
        ...
        //Set The label Text
        ElementToSet.find('label.TradesDetails').text(TradeTypeWord + ' ' + Vol + ' Lot');
        ElementToSet = null;
    }
    ...
    function CheckTradesData() {
        ...
        if (OHLCChart.ChartSettings.IsOpenTradeVisible) {
            var OpenTradesDateLength = WC.TM.TradesModuleVM.openTrades().length;
            ...
            OpenTradesSub = WC.TM.TradesModuleVM.openTrades.subscribe(SubscribeToTradesAndAddDataToLocalVisibleTradesOnChart);
        }
        else {
            if (OpenTradesSub !== null) {
                OpenTradesSub.dispose();
    ...
    function SubscribeToTradesAndAddDataToLocalVisibleTradesOnChart(NewValue) {
        var OpenTradesDataLength = WC.TM.TradesModuleVM.openTrades().length;
        ...
        if (isReCreateData) {
            ...
            for (var i = 0; i < OpenTradesDataLength; i++) {
                var IsDuplicate = false;
                if (WC.TM.TradesModuleVM.openTrades()[i].Symbol() === OHLCChart.ChartSettings.Symbol) {
                    if (parseFloat(WC.TM.TradesModuleVM.openTrades()[i].OpenQuote()) >= OHLCChart.ComputationProperties.yMin && parseFloat(WC.TM.TradesModuleVM.openTrades()[i].OpenQuote()) <= OHLCChart.ComputationProperties.yMax) {
                        if (TradesDataLength === 0) {
                            OpenTradesData.push(WC.TM.TradesModuleVM.openTrades()[i]);
                            if (OHLCChart.ChartSettings.IsLineStyle) {
                                SetElementsForTrades(WC.TM.TradesModuleVM.openTrades()[i].TradeId());
                            }
                            continue;
                        }
                        ...
                        for (var ii = 0; ii < TradesDataLength; ii++) {
                            if (parseFloat(OpenTradesData[ii].OpenQuote()) === parseFloat(WC.TM.TradesModuleVM.openTrades()[i].OpenQuote())) {
                                IsDuplicate = true;
                                break;
                            }
                        }
    ...
    function DrawDashedLineTrades() {
        ...
        for (var i = 0; i < OpenTradesDataLength; i++) {
            ...
            OHLCChart.drawDashedLine("Trades", 0, YPositionLine, OHLCChart.ComputationProperties.width, YPositionLine, [6, 7, 0], LineColor, 1.5);
            SetPositionTradesAndOrders(OpenTradesData[i].TradeId(), YPositionLine, LineColor, TradeTypeWord, OpenTradesData[i].Volume());
        }
        if (WC.TM.TradesModuleVM.openTrades().length !== 0) {
    ...
    OHLCChart.GetAllSettingsProperty = function () {
        return {
            ...
            BuysColor: OHLCChart.ChartSettings.BuysColor,
            ...
            Sells: OHLCChart.ChartSettings.Sells,
    ...
    OHLCChart.GetColorSchemeObjects = function () {
        ...
        var PropertyaGridObject = {
            ...
            BuysColor: { group: GroupName, name: "Buys Color", type: "color", options: { preferformat: "hex" } },
...
WC.CM.ChartSettingsProperty = function (settings) {
    ...
    //Chart Buys Color
    self.BuysColor = settings.BuysColor;
    //Chart ForeGround
...
WC.CM.BottomIndicatorChart = function (Container, ChartFormResizingMethod, MainChartInstance, IndicatorID, ChartDisplay, ContainerID) {
    ...
    function SetAllChartEvents() {
        //~~~Chart Events~~~
        $(IndicatorChart.ParentELement).find('.IndicatorTopCanvas').removeAttr('style');
        $(IndicatorChart.ParentELement).find('.IndicatorTopCanvas').attr('style', 'position: absolute; left: 0px; z-index: 10; top: 0px;');

        $(IndicatorChart.ParentELement).find('.IndicatorTopCanvas').mousedown(function (e) {
        ...
        $(IndicatorChart.ParentELement).find('.IndicatorTopCanvas').mouseup(function () {
        ...
        $(IndicatorChart.ParentELement).find('.IndicatorTopCanvas').mousemove(function (e) {
            var elem = $(IndicatorChart.ParentELement).offset();
            var X = e.pageX - elem.left;
        ...
        $(IndicatorChart.ParentELement).find('.IndicatorTopCanvas').mouseout(function () {
            IndicatorChart.ChartValidationsProperties.stillDown = false;
            HideChartMarkers();
