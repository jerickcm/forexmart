    ...
    <div class="section">
        ...
        <div class="tab-content acct-cont admin-tab-cont">
            <div role="tabpanel" class="row tab-pane active" id="tab3">
                    ...
                    <table class="table table-striped queue-tab">
                        ...
                        <?php  if($user_documents){
                            ...
                            foreach($user_documents as $key => $value) {
                                ...
                                echo '<td><a href="#'.$value['id'].'"  data-userid="'.$value['user_id'].'" data-id="'.$value['id'].'" data-fullname="'.$value['full_name'].'" data-email="'.$value['email'].'" data-address="'.$value['street'].','.$value['city'].','.$value['state'].','.$this->g_m->getCountries($value['country']).','.$value['zip'].'" class="queue-action declined" data-toggle="modal" data-target="#declineStat">'.lang("vd").'</a></td>';
                                echo '</tr>';
                                $no=$no+1;
                            }
                        }else{
                ...
                <div class="col-md-6">
                    <?= form_open('administration/accountverification_declineddocuments',array('id' => 'PerPageForm','class'=> 'form-inline'),''); ?>
                    <div class="form-group">
                        <label  class="number"><?= lang('norec');?></label>
    ...
    <div class="modal-dialog modal-lg round-0">
        <div class="modal-content round-0">
            <div class="modal-header round-0">
                ...
                <h4 class="modal-title"><?= lang('acct-dd');?></h4>
            </div>
                    ...
                    <div class="col-sm-6">
                        <form class="form-horizontal">
                            <div class="form-group acct-grp">
                                <label  class="col-sm-5 control-label acct-lbl"><?= lang('an');?></label>
                                <div class="col-sm-7">
                                    <p class="acct-val accountnumber"></p>
                                </div>
                            </div>
                            <div class="form-group acct-grp">
                                <label  class="col-sm-5 control-label acct-lbl"><?= lang('fn');?></label>
                                <div class="col-sm-7">
                                    <p class="acct-val fullname"></p>
                                </div>
                            </div>
                            <div class="form-group acct-grp">
                                <label  class="col-sm-5 control-label acct-lbl"><?= lang('Em');?></label>
                                <div class="col-sm-7">
                                    <p class="acct-val email"></p>
                                </div>
                            </div>
                            <div class="form-group acct-grp">
                                <label  class="col-sm-5 control-label acct-lbl"><?= lang('Addr');?></label>
                                <div class="col-sm-7">
                                        ...
                                        foreach ($arrykeyholder as &$usearraykey) {
                                            foreach ($result[$usearraykey] as $key => $value) {
                                                if ($value['doc_type']==0 or $value['doc_type']==1){
                                                    ...
                                                    echo '<td>' .$status . '</td>';
                                                    echo '<td>' . $value['date_uploaded'] . '</td>';
                                                    echo '<td><a href="#" id="declined-nth-approve'.$value['id'].'"  data-id="'.$value['id'].'" class="declined-nth-approve queue-action"  data-target="#viewAcct">'.lang('modal_apprv').'</a></td>';
                                                    echo '</tr>';
                                                    echo '<tr id="collapseme'.$value['id'].'" class="collapse"><td colspan="4" ><div>';
                                                    echo '<img id="img'.$value['id'].'" src="" lsrc="'.$this->config->item('domain-my').'/assets/user_docs/'.$value['file_name'].'" class="img'.$value['user_id'].' centroid fimage img-responsive acct-ver-img"/>';
                                                    echo '</div></td></tr>';
                                                }
                                            }
                                        }
...
<script type="text/javascript">
    $(document).on("click", ".hitdisplay", function () {
        ...
        var lsrc = $('#img'+$(this).data('id')).attr('lsrc');
        $('#img'+$(this).data('id')).attr('src',lsrc);
    ...
    $(document).on("click", ".modal-nth-decline", function () {
        var Id = $(this).data('id');
        $("#DocId").val(Id);
        $("#location").val('accountverification_declineddocuments');
        $('#declineStat').modal('hide');
    });
    //    modal decline document
    $(document).on("click", ".declined-nth-approve", function () {
        var Id = $(this).data('id');
        var prvt = [];
        prvt["data"] = {
            id: Id,
        ...
        pblc['request'].done(function( data ) {
            ...
            window.location.href = site_url+'administration/accountverification_declineddocuments';
        });
