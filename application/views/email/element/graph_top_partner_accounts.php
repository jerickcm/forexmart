    ...
    <div class="row">
        <div class="main-dashboard-holder">
            <row>
                ...
                $view_data = array(
                    'days' => $days,
                    'top_country' => $top_partner_country,
                    'total_country_count' => $total_partner_country_count
                );
                echo $this->load->view('email/element/top_partner_accounts_widget', $view_data, true);
                ?>
            </row>
        </div>
    </div>
</div>
<form method="POST" enctype="multipart/form-data" id="topPartnerGraphForm">
    <input type="hidden" name="img_val_30" id="img_top_partner_val_30" value="" />
    <input type="hidden" name="img_val_60" id="img_top_partner_val_60" value="" />
    <input type="hidden" name="img_val_180" id="img_top_partner_val_180" value="" />
    <input type="hidden" name="img_val_360" id="img_top_partner_val_360" value="" />
    <input type="hidden" name="img_val_540" id="img_top_partner_val_540" value="" />
</form>
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        accounts_option.chart.renderTo = 'top_partner_account_graph';
        var top_partner_chart = new Highcharts.Chart(accounts_option);
        top_partner_chart.setTitle({text: 'Top 10 Countries of Partner Accounts Graph'});
        <?php if(count($top_partner_country) > 0){ ?>
        <?php foreach($top_partner_country as $key => $partner_country){ ?>
        top_partner_chart.addSeries({
            animation: false,
            name: "<?php echo $partner_country['name'] ?>",
            color: "<?php echo $color_array[$key] ?>",
            data: [<?php echo implode(",", $partner_country_count[$partner_country['country']]) ?>]
        });
        <?php } ?>
        <?php } ?>
