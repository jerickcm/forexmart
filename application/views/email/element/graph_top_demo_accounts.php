        ...
        <div class="main-dashboard-holder">
            <row>
                <?php
                    $view_data = array(
                        'days' => $days,
                        'top_country' => $top_demo_country,
                        'total_country_count' => $total_demo_country_count
                    );
                    echo $this->load->view('email/element/top_demo_accounts_widget', $view_data, true);
                ?>
            </row>
        </div>
    </div>
</div>
<form method="POST" enctype="multipart/form-data" id="topDemoGraphForm">
    <input type="hidden" name="img_val_30" id="img_top_demo_val_30" value="" />
    <input type="hidden" name="img_val_60" id="img_top_demo_val_60" value="" />
    <input type="hidden" name="img_val_180" id="img_top_demo_val_180" value="" />
    <input type="hidden" name="img_val_360" id="img_top_demo_val_360" value="" />
    <input type="hidden" name="img_val_540" id="img_top_demo_val_540" value="" />
</form>
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        accounts_option.chart.renderTo = 'top_demo_account_graph';
        var top_demo_chart = new Highcharts.Chart(accounts_option);
        top_demo_chart.setTitle({text: 'Top 10 Countries of Demo Accounts Graph'});
        <?php if(count($top_demo_country) > 0){ ?>
        <?php foreach($top_demo_country as $key => $demo_country){ ?>
        top_demo_chart.addSeries({
            animation: false,
            name: "<?php echo $demo_country['name'] ?>",
            color: "<?php echo $color_array[$key] ?>",
            data: [<?php echo implode(",", $demo_country_count[$demo_country['country']]) ?>]
        });
        <?php } ?>
        <?php } ?>
