        ...
        <div class="main-dashboard-holder">
            <row>
                <?php
                    $view_data = array(
                        'days' => $days,
                        'top_country' => $top_real_country,
                        'total_country_count' => $total_real_country_count
                    );
                    echo $this->load->view('email/element/top_real_accounts_widget', $view_data, true);
                ?>
            </row>
        </div>
    </div>
</div>
<form method="POST" enctype="multipart/form-data" id="topRealGraphForm">
    <input type="hidden" name="img_val_30" id="img_top_real_val_30" value="" />
    <input type="hidden" name="img_val_60" id="img_top_real_val_60" value="" />
    <input type="hidden" name="img_val_180" id="img_top_real_val_180" value="" />
    <input type="hidden" name="img_val_360" id="img_top_real_val_360" value="" />
    <input type="hidden" name="img_val_540" id="img_top_real_val_540" value="" />
</form>
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        accounts_option.chart.renderTo = 'top_real_account_graph';
        var top_real_chart = new Highcharts.Chart(accounts_option);
        top_real_chart.setTitle({text: 'Top 10 Countries of Real Accounts Graph'});
        <?php if(count($top_real_country) > 0){ ?>
        <?php foreach($top_real_country as $key => $real_country){ ?>
        top_real_chart.addSeries({
            animation: false,
            name: "<?php echo $real_country['name'] ?>",
            color: "<?php echo $color_array[$key] ?>",
            data: [<?php echo implode(",", $real_country_count[$real_country['country']]) ?>]
        });
        <?php } ?>
        <?php } ?>
