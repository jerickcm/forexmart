                        ...
                        <div role="tabpanel" class="tab-pane active" id="tab1">
                            <div class="table-responsive">
                                <table class="table table-stripped calendar-tab table-hover">
                                    ...
                                    <?php
                                    $cnt = count($xml->event);
                                    // var_dump($cnt);date_format($date,"H:i")
                                    ?>
                                    <tr>
                                        <td colspan="7" class="ec-date"><?=$xml->event[0]->date;?></td>
                                    </tr>
                                    ...
                                    for($i=0; $i<$cnt; $i++)
                                    {
                                       // $url 	= $xml->weeklyevents->event[$i]->link;
                                        $title 	= $xml->event[$i]->title;
                                       // $desc = $xml->weeklyevents->event[$i]->description;

                                        $country = $xml->event[$i]->country;
                                        $previous = $xml->event[$i]->previous;
                                        $impact = $xml->event[$i]->impact;
                                        $forecast = $xml->event[$i]->forecast;
                                       // $actual = $xml->weeklyevents->event[$i]->actual;
                                        $date = $xml->event[$i]->date;
                                        $time = $xml->event[$i]->time;
                                        ?>

                                        <tr>
                                            <td><?=$time;?></td>
                                            <td class="f32"><i class="flag <?=substr(strtolower($country),0,2)?>"></i> <span class="country-cur"><?=$country?></span></td>
                                            <td>
                                                <div class="progress calendar-progress">
                                                    <div class="progress-bar progress-bar-<?=strtolower($impact)?> <?=strtolower($impact)?>" role="progressbar" aria-valuenow="" aria-valuemin="0" aria-valuemax="100">
                                                    </div>
                                                </div>
                                            </td>
