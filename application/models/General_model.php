...
//        $this->db->or_where("u.".$field2, 2);
    ...
    function updatemysecond($table, $field, $id, $field2, $id2, $field3, $id3, $data)
    {
        $this->db->where($field, $id);
    ...
    function updatemycustom($table, $field1, $id1, $field2, $id2, $data)
    {
        $this->db->where($field1, $id1);
        $this->db->where($field2, $id2);
        $this->db->where('doc_type', 0);
        $this->db->or_where('doc_type', 1);
        $this->db->update($table, $data);
        if ($this->db->affected_rows() > 0) {
            return true;
        }
    ...
    function showssinglesecond($table, $field1 = "", $id1 = "", $field2 = "", $id2 = "", $field3 = "", $id3 = "", $select = "", $order_by = "")
    {
    ...
    function convertDtime($date_time, $type = "")
    {
        ...
        $get_date = $exp_date_time[0];
        $get_time = $exp_date_time[1];
...
//            'BGN' =>'bg', 
        ...
        if ($country_id) {
            return isset($flageArray[$country_id]) ? $flageArray[$country_id] : false;
        } else {
            return $flageArray;
        }
    }
...
function sendEmailFiled($file_name, $subject, $email, $data,$configemail=null,$from=null)
    {
        $this->config->load('tank_auth');
        $this->load->library('email');
        if($configemail != null){
        ...
        $this->SMTPDebug =1;
        $this->email->from(($from)?$from:'noreply@mail.forexmart.com','forexmart');
        $this->email->reply_to($this->config->item('webmaster_email', 'tank_auth'), $this->config->item('website_name', 'tank_auth'));
        $this->email->to($email);
        $this->email->subject($subject);
        $this->email->message($this->load->view('email/'.$file_name, $data, TRUE));
    ...
    public function getPartnershipCurrency_v2($id = null)
    {
        $data = array(
            'EUR' => "EUR",
            'USD' => "USD",
    ...
    function getLeverage($id = null, $limit = 5000)
    {
...
//        }
    ...
    function getPartnersStatusType($id = null)
    {
       // if(IPLoc::Office()){
            $data = array(
                '1' => lang('partnershipstatus_2'),
                '2' => lang('partnershipstatus_3')
            );
        /*}else{
            $data = array(
                '1' => "Individual",
...
//            '#LLOY'=> '#LLOY',
    ...
    function getGroupSpard($id = null)
    {
        ...
        $data = array(
            ...
            "refStC301"=> "ForexMart Macro Standard / spread 3.0 pips",
        );
    ...
    public function getGroupCurrency($account_type, $currency_code, $swap_free = 0)
    {
        ...
        if(in_array($account_type, array(1,2,4))){
            if($this->session->userdata('isChina') == '1'){
                ...
                return $data[$account_type][$currency_code][$swap_free].'-cn';
            } else {
            ...
            if($this->session->userdata('isChina') == '1'){
                $this->session->unset_userdata('isChina');
                return $data[$account_type][$currency_code].'-cn';
            } else {
    ...
    public function getDemoGroupCurrency($account_type, $currency_code, $swap_free = 0){
        ...
        $data = array(
            ...
            4 => array( // API group codes for ForexMart Demo Micro
                'USD' => 'StUSC',
                'EUR' => 'StEUC'
            )
        );
    ...
    public function getGMT($gmt_id = '')
    {
        ...
        if ($gmt_id) {
            return isset($currency[$gmt_id]) ? $currency[$gmt_id] : false;
        } else {
            return $gmt;
        }
    }
    ...
    public function showdoc2($table, $field1, $id1, $field2, $id2, $select)
    {
        $this->db->select($select);
        $this->db->from($table);
        $this->db->where($field1, $id1);
    ...
    function updatemyNDB($table, $field, $id, $data)
    {
        $this->db->where($field, $id);
        $this->db->update($table, $data);
        if ($this->db->affected_rows() > 0) {
    ...
    public function showAllAccountnumber2()
    {
        $this->db->select('account_number');
        $this->db->from('mt_accounts_set' . ' mts');
        $this->db->where("CHARACTER_LENGTH(mts." . 'account_number) >', 5);
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            return $query->result_array();
        }
    ...
    public function showAllAccountnumber()
    {
        $this->db->select('account_number');
        $this->db->from('mt_accounts_set' . ' mts');
        $this->db->join('users' . ' u', ' u.id = mts.user_id');
    ...
    public function showAllAccountnumberMTS()
    {
        $this->db->select('account_number');
        $this->db->from('mt_accounts_set' . ' mts');
        $this->db->where("CHARACTER_LENGTH(mts." . 'account_number) >', 4);
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            return $query->result_array();
        }
    ...
    public function showAllAccountnumberMTS2()
    {
        $this->db->select('account_number');
        $this->db->from('mt_accounts_set' . ' mts');
        $this->db->where("CHARACTER_LENGTH(mts." . 'account_number) >', 4);
        $this->db->where("mts." . 'group', '');
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            return $query->result_array();
        }
    ...
    public function showAllAccountnumberMTS3()
    {
        $this->db->select('account_number');
        $this->db->from('mt_accounts_set' . ' mts');
        $this->db->where("CHARACTER_LENGTH(mts." . 'account_number) >', 4);
        $this->db->where("mts." . 'group', 'service er');
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            return $query->result_array();
        }
    ...
    public function showAllAccountnumberNDB()
    {
        $this->db->select('account_number');
        $this->db->from('mt_accounts_set' . ' mts');
        $this->db->join('users' . ' u', ' u.id = mts.user_id');
        $this->db->where("u." . 'type', '1');
        $this->db->where("u." . 'nodepositbonus', '1');
        $this->db->where("CHARACTER_LENGTH(mts." . 'account_number) >', 5);
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            return $query->result_array();
    ...
    public function chechkId($table, $field, $id)
    {
        $this->db->select("*");
        $this->db->from($table);
        $this->db->where($field, $id);
...
//        }
    ...
    function showt1w1o1($table, $field, $id, $select, $order_by, $order, $limit)
    {
        $this->db->select($select);
        $this->db->from($table);
        $this->db->where($field, $id);
    ...
    function showsfields2($table, $select = "")
    {
    ...
    public function getQueryStirngResult($table,$star,$data="",$limit="",$gropup="",$order="",$join=""){
        $this->db->select($star);
        $this->db->from($table);
	 ...
	 public function getQueryStirngResultJoin($table,$star,$data="",$gropup="",$order="",$join=""){
        $this->db->select($star);
        $this->db->from($table);
       if($join!="")
        {
            ...
            {   
                $tabJoy=  explode("___", $key);
                $this->db->join($tabJoy[0] ,$val,$tabJoy[1]);
            }        
        }  // joint  array('table___left'=>user.id=info.user_id)
     ...
     public function getQueryStirngRowJoin($table,$star,$data="",$gropup="",$order="",$join=""){
        $this->db->select($star);
        $this->db->from($table);
        if($join!=""){
            foreach($join as $key=>$val)
...
//"YU" => "Yugoslavia",
    ...
    function updateConditional($table, $condition, $data)
    {
    ...
    function show_all($table, $field = "", $id = "", $select = "") {
    ...
    {
        FXPP::CI()->config->load('tank_auth');
        FXPP::CI()->load->library('email');
        if ($configemail != null) {
            FXPP::CI()->email->initialize($configemail);
        }
        FXPP::CI()->SMTPDebug = 1;
        FXPP::CI()->email->from($this->config->item('webmaster_email', 'tank_auth'), FXPP::CI()->config->item('website_name', 'tank_auth'));
        FXPP::CI()->email->reply_to($this->config->item('webmaster_email', 'tank_auth'), FXPP::CI()->config->item('website_name', 'tank_auth'));
        FXPP::CI()->email->to($email);
        FXPP::CI()->email->subject($subject);
        FXPP::CI()->email->message(FXPP::CI()->load->view('email/' . $file_name, $data, TRUE));
        if ( ! FXPP::CI()->email->send())
        {
            return false;
        }
    ...
    public function getClientByDate($from,$to){
        $this->db->select('*');
        $this->db->from('mailer_periodic');
        $this->db->where('DATE(DateAvailable) >=',$from);
        $this->db->where('DATE(DateAvailable) <=',$to);
        $this->db->where('Tag',0);
        $this->db->where('Counter_to_staging',0);
        $this->db->limit(90);
    ...
    public function GetRecipientByDate($from,$to){
        $this->db->select('m_t.Id,m_t.Email,m_t.Date,m_t.Language,m_t.recipient_type,m_t.Fullname,m_t.unsubscribekey,m_t.counter');
        $this->db->from('mailer_periodic as m_p');
        $this->db->join('mailer_test_recipients as m_t','m_p.RecipientId = m_t.Id','inner');
        $this->db->where('DATE(m_p.DateAvailable) >=',$from);
        $this->db->where('DATE(m_p.DateAvailable) <=',$to);
        $this->db->where('m_p.Tag',0);
        $this->db->where('m_p.Counter_to_staging',0);
        $this->db->group_by('m_p.RecipientId');
        $this->db->limit(90);
    ...
    /**
     * UPDATE MT GROUPS FOR MICRO ACCOUNT
     *
     * @param	int, array
     *
     */
    function update_mt_group($user_id, $usergroup){
        //$this->db->trans_start();
        $this->db->set('group', $usergroup);
        $this->db->where('user_id', $user_id);
        $this->db->update('mt_accounts_set');
        //$this->db->trans_complete();
        if ($this->db->affected_rows() > 0){
            return true;
        }
    ...
    /**
     * SET MICRO TO 1 FOR MICRO ACCOUNT
     *
     * @param	int, array
     *
     */
    ...
    function selectOptionList_cpa($data, $selected_val = null)
    {
        $selectOption = "";
        if (is_array($data)) {
            ...
            foreach ($data as $key => $d) {
                ...
                if($selected=="selected"){
                    $selectOption = $selectOption . "<option " . $selected . " value='" . $key . "'>&#8239;</option>";
                }else{
                    $selectOption = $selectOption . "<option " . $selected . " value='" . $key . "'>" . $d . "</option>";
                }
    ...
    function show_emailTracker($email,$unsubscribekey)
    {
        $this->db->select('*');
        $this->db->from('email_tracker');
        $this->db->where('email', $email);
        $this->db->where('unsubscribekey', $unsubscribekey);
        $data = $this->db->get();
        if ($data->num_rows() > 0) {
            return $data->result_array();
        } else {
    ...
    function checkEmailTracker($email,$email_id)
    {
        $this->db->select('*');
        $this->db->from('emailstats_tradeoffer');
        $this->db->where('email', $email);
    ...
    function show_mailer_periodic_emailTracker($date,$type){
        $this->db->select('count(*) as counter');
        $this->db->from('mailer_periodic');
        $this->db->where('DATE(DateAvailable) >=',$date);
        $this->db->where('Counter',1);
        $this->db->where('Unsubscribe',0);
        if($type == '1'){
            $this->db->like('MethodName', 'partner');
        }else{
            $this->db->not_like('MethodName', 'partner');
        }
        $data = $this->db->get();
        if ($data->num_rows() > 0) {
            return $data->result_array();
    ...
    function show_emailTracker_mailerPeriodic($type){
        $this->db->select('count(*) as counter_email_tracker');
        $this->db->from('email_tracker');
        if($type == '1'){
            $this->db->like('methodname', 'partner');
        }else{
    ...
    public function show_referral(){
        $this->db->select('a.user_id , a.account_number , b.affiliate_code , b.referral_affiliate_code');
        $this->db->from('mt_accounts_set a');
        $this->db->join('users_affiliate_code b','a.user_id = b.users_id');
        $this->db->where('referral_affiliate_code','BHOOQ');
        $data = $this->db->get();
        if ($data->num_rows() > 0) {
            return $data->result_array();
        } else {
    ...
    function showssingle_query($user_id){
        $sql = "Select `account_number` from (SELECT
    ...
    function rowdeletfield2($table,$field,$id,$field2,$id2){
        $this->db->where($field, $id);
        $this->db->where($field2, $id2);
    ...
    function getAllTeamStatistics($title = null,$data,$period){
        $this->db->from('team_page_logs');

        if ($period == 'weekly' || $period == 'monthly') {
            $this->db->where('DATE(date) >=',$data['from']);
            $this->db->where('DATE(date) <=',$data['to']);
        } else {
    ...
    function getReglog($limit,$offset)
    {
        $this->db->select('*');
        $this->db->from('registration_logs');
        $this->db->ORDER_BY('id','DESC');
    ...
    function whereCondition($table, $conditon,$select="*",$order_by=null){
        $this->db->select($select);
        $this->db->from($table);
        $this->db->where($conditon);
        if(!is_null($order_by)){ $orderby = explode(',',$order_by); $this->db->order_by($orderby[0],$orderby[1]); }
        $data = $this->db->get();
        if($data->num_rows() > 0) {
            return $data->row_array();
        }else{
    ...
    function updatemyfirst($table, $field, $id, $field2, $id2, $data)
    {
        $this->db->where($field, $id);
        $this->db->where($field2, $id2);
        $this->db->update($table, $data);
