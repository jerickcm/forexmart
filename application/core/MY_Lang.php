...
class MY_Lang extends CI_Lang {
  ...
  private $languages = array(
      ...
      "gr" => "greek",
      "cs" => "Czech",
      "zh" => "chinese"//FOR CONTEST RULES PAGE ONLY
  );
...
//      echo $language;
    ...
    if ((!$url_ok) && (!$this->is_special($uri_segment['parts'][0]))) // special URI -> no redirect
    {
      ...
      $CFG->set_item('language', $this->languages['en']);
      ...
      $new_url = $CFG->config['base_url'].$index_url.$this->default_lang().'/'.$uri.$getAffiliateCode;

//      echo "Redirect triggered to ".$this->uri;
