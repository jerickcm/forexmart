...
class Tank_auth
{
	...
	function login($login, $password, $remember, $login_by_username, $login_by_email)
	{
		if ((strlen($login) > 0) AND (strlen($password) > 0)) {
			...
			if (!is_null($users = $this->ci->users->$get_user_func($login))) {	// login ok
...
//                }
	...
	function checkUserExist($login, $password, $login_by_username, $login_by_email, $login_type = 0)
	{
		if ((strlen($login) > 0) AND (strlen($password) > 0)) {
			...
			if (!is_null($users = $this->ci->users->$get_user_func($login, $login_type))) {	// login ok
				...
				$this->error = array('username' => 'Incorrect login');
			}
		}
		return FALSE;
	}
